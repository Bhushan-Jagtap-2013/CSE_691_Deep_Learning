...
class TwoLayersNN (object):
    ...
    def __init__ (self, inputDim, hiddenDim, outputDim):
        ...
        # initialize w1 and b1 for 1st layer

        self.params['w1'] = np.random.normal(0.0001, 0.0001, (inputDim, hiddenDim))
        # self.params['b1'] = np.random.normal(0.0001, 0.0001, (1, hiddenDim))
        self.params['b1'] = np.ones(hiddenDim)

        # initialize w2 and b2 for second layer

        self.params['w2'] = np.random.normal(0.0001, 0.0001, (hiddenDim, outputDim))
        # self.params['b2'] = np.random.normal(0.0001, 0.0001, (1, outputDim))
    ...
    def calLoss (self, x, y, reg):
        ...
        # layer 1 : compute score for layer 1

        h_prime = x.dot(w1) + b1
        h = np.maximum(0.01 * h_prime, h_prime)

        # layer 2 : compute score for layer 2
        ...
        # find db2
        grads['b2'] = np.sum(ds_prime, axis = 0) # / h.shape[0]
        ...
        grads['b1'] = np.sum(dh_prime, axis = 0) #/ x.shape[0]
