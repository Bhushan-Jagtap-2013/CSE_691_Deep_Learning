...
sample_lr = [5e-08, 0.5e-7, 0.7e-7, 0.1e-7, 3e-3]
sample_reg = [4.6e4, 2.8e4, 4e4, 7.2e4, 5.4e4, 1.6e4, 9e-3]

# Take few sample values and train classifier using them
# and test that classifier for validation set
# keep track of smallest accuracy  and  corresponding parameter

for i in sample_lr:
    for j in sample_reg:
        classifier = TwoLayersNN(xTrain.shape[1], hiddenNeurons, numClasses)
        classifier.train(xTrain, yTrain, lr=i, reg=j, iterations=1500, verbose=False)
        temp_acc = classifier.calAccuracy(xVal, yVal)
